...
class FileNameCryptorImpl implements FileNameCryptor {
	...
	private static final ThreadLocal<SivMode> SM4_SIV = new ThreadLocal<SivMode>() {
		@Override
	...
	public String hashDirectoryId(String cleartextDirectoryId) {
		...
		byte[] encryptedBytes = SM4_SIV.get().encrypt(encryptionKey, macKey, cleartextBytes);
	...
	public String encryptFilename(BaseEncoding encoding, String cleartextName, byte[]... associatedData) {
		...
		byte[] encryptedBytes = SM4_SIV.get().encrypt(encryptionKey, macKey, cleartextBytes, associatedData);
	...
	public String decryptFilename(BaseEncoding encoding, String ciphertextName, byte[]... associatedData) throws AuthenticationFailedException {
		try {
			...
			byte[] cleartextBytes = SM4_SIV.get().decrypt(encryptionKey, macKey, encryptedBytes, associatedData);

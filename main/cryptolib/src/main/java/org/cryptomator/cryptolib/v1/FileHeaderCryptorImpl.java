...
class FileHeaderCryptorImpl implements FileHeaderCryptor {
	...
	public ByteBuffer encryptHeader(FileHeader header) {
		...
		try {
			...
			// encrypt payload:
			Cipher cipher = CipherSupplier.SM4_CTR.forEncryption(headerKey, new IvParameterSpec(headerImpl.getNonce()));
	...
	public FileHeader decryptHeader(ByteBuffer ciphertextHeaderBuf) throws AuthenticationFailedException {
		...
		try {
			// decrypt payload:
			Cipher cipher = CipherSupplier.SM4_CTR.forDecryption(headerKey, new IvParameterSpec(nonce));

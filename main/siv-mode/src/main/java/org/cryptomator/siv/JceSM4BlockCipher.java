 ...
 * JCE doesn't allow us to retrieve the plain cipher without a mode, so we explicitly request {@value #SINGLE_BLOCK_PLAIN_SM4_JCE_CIPHER_NAME}.
 ...
 */
final class JceSM4BlockCipher implements BlockCipher {

	private static final String ALG_NAME = "SM4";
	private static final String KEY_DESIGNATION = "SM4";
	private static final String SINGLE_BLOCK_PLAIN_SM4_JCE_CIPHER_NAME = "SM4/ECB/NoPadding";
	...
	JceSM4BlockCipher(Provider provider) {
		try {
			if(provider != null) {
				this.cipher = Cipher.getInstance(SINGLE_BLOCK_PLAIN_SM4_JCE_CIPHER_NAME, provider);
			} else {
				this.cipher = Cipher.getInstance(SINGLE_BLOCK_PLAIN_SM4_JCE_CIPHER_NAME); // defaults to SunJCE
			}
		} catch (NoSuchAlgorithmException | NoSuchPaddingException e) {
			throw new IllegalStateException("Every implementation of the Java platform is required to support SM4/ECB/NoPadding.");
		}
	}

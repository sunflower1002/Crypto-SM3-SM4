...
public final class SivMode {
	...
	/**
	 * Creates an SM4-SIV instance using JCE's cipher implementation, which should normally be the best choice.<br>
	 * <p>
	 * For embedded systems, you might want to consider using {@link #SivMode(BlockCipherFactory)} with BouncyCastle's {@code SM4LightEngine} instead.
	 *
	...
	/**
	 * Creates an SM4-SIV instance using a custom JCE's security provider<br>
	 * <p>
	 * For embedded systems, you might want to consider using {@link #SivMode(BlockCipherFactory)} with BouncyCastle's {@code SM4LightEngine} instead.
	 *
	...
	public SivMode(final Provider jceSecurityProvider) {
		this(ThreadLocal.withInitial(() -> new JceSM4BlockCipher(jceSecurityProvider)), new JceSM4CtrComputer(jceSecurityProvider));
	}

	/**
	 * Creates an instance using a specific Blockcipher.get(). If you want to use SM4, just use the default constructor.
	 *

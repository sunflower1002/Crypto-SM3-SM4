...
/**
 * Performs CTR Mode computations facilitating a cipher returned by JCE's <code>Cipher.getInstance("SM4/CTR/NoPadding")</code>.
 */
final class JceSM4CtrComputer implements SivMode.CtrComputer {
	...
	public JceSM4CtrComputer(final Provider jceSecurityProvider) {
		this.threadLocalCipher = new ThreadLocal<Cipher>(){
			@Override
			protected Cipher initialValue() {
				try {
					if (jceSecurityProvider == null) {
						return Cipher.getInstance("SM4/CTR/NoPadding");
					} else {
						return Cipher.getInstance("SM4/CTR/NoPadding", jceSecurityProvider);
					}
				} catch (NoSuchAlgorithmException | NoSuchPaddingException e) {
					throw new IllegalStateException("SM4/CTR/NoPadding not available on this platform.", e);
				}
			}
		};
	}
